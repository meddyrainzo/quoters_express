...
export default class QuotesService {
    ...
    private async createQuoteResponse(quote: IQuote, userId: string): Promise<SingleQuoteResponse> {
    ...
    private mapQuoteToSingleQuoteResponse(quote: IQuote, postedBy: PostedBy, userId?: string): SingleQuoteResponse {
        ...
        return new SingleQuoteResponse(id, quote.quote, quote.author, postedBy,
             quote.posted_on!.toDateString(), likesCount, likedByYou, quote.comments ?? 0);
    }
    ...
    async getSingleQuote(quoteId: string, userId?: string): Promise<Result<SingleQuoteResponse>> {
        try {
            ...
            const singleQuoteResponse = await this.createQuoteResponse(quote, userId!);
    ...
    async writeQuote(request: WriteQuoteRequest, userId: string): Promise<Result<SingleQuoteResponse>> {
        try {
            ...
            const singleQuoteResponse = await this.createQuoteResponse(createdQuote, userId!);
            return { tag: 'success', result: singleQuoteResponse };
        } catch(err) {
    ...
    async changeQuote(request: ChangeQuoteRequest, quoteId: string, userId: string): Promise<Result<SingleQuoteResponse>> {
        try {
            ...
            const singleQuoteResponse = await this.createQuoteResponse(updatedQuote!, userId!);
            return { tag: 'success', result: singleQuoteResponse };
        } catch(err) {
