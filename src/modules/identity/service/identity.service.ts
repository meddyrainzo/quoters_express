...
export default class IdentityService {
  async registerUser(
    ...
    try {
      ...
      return {
        tag: 'success',
        result: new RegisterUserResponse(user._id.toString()),
      };
    } catch (err) {
      logger.error('An error occured when registering the user', {
        error_message: err.message,
      });
      return {
        tag: 'failure',
        error: new ErrorResponse(400, 'User already exists'),
      };
    }
  }
  ...
  async loginUser(
    ...
    const invalidLoginError = {
      tag: 'failure',
      error: new ErrorResponse(404, IdentityErrorReason.INVALID_LOGIN),
    } as Failure;
    try {
      ...
      return {
        tag: 'failure',
        error: new ErrorResponse(400, 'Failed to log in user'),
      };
    }
  }
  ...
  async changePassword(
    ...
    try {
      ...
      if (!user) {
        ...
        return {
          tag: 'failure',
          error: new ErrorResponse(400, 'Error changing password'),
        };
      }
      // Validate the password
      const isPasswordValid = await PasswordHasher.verifyPassword(
      ...
      if (!isPasswordValid) {
        ...
        return {
          ...
          error: new ErrorResponse(
            400,
            IdentityErrorReason.INCORRECT_OLDPASSWORD
          ),
        };
      }
      ...
      logger.error('An error occured during password change', {
        error_message: err.message,
      });
      return {
        tag: 'failure',
        error: new ErrorResponse(400, 'Failed to change the password'),
      };
    }
  }
}
